...
def train():
    ...
    model = CenterNet(input_data, is_training, cfg.net)
    hm = model.pred_hm
    wh = model.pred_wh
    reg = model.pred_reg
    ...
    with tf.Session() as sess:
        ...
        #     saver.restore(sess, './checkpoint/centernet_test_loss=4.4497.ckpt-7')
        for epoch in range(1, 1 + cfg.epochs):
            ...
            for i in pbar:
                _, summary, train_step_loss, global_step_val, _hm_loss, _wh_loss, _reg_loss = sess.run(
                    [train_op, write_op, total_loss, global_step, hm_loss, wh_loss, reg_loss], feed_dict={is_training: True})
                train_epoch_loss.append(train_step_loss)
                summary_writer.add_summary(summary, global_step_val)
                pbar.set_description("train loss: %.2f hm_loss: %.2f wh_loss:%2f reg_loss:%2f" % train_step_loss, _hm_loss, _wh_loss, _reg_loss)
