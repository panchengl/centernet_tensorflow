...
def process_data(line, use_aug):
    ...
    ori_w = s[2]
    ori_h = s[3]
    labels = np.array([list(map(lambda x: int(float(x)), box.split(','))) for box in s[4:]])
...
def get_data(batch_lines, use_aug):
    ...
    batch_hm = np.zeros(
        (cfg.batch_size, cfg.input_image_h // cfg.down_ratio, cfg.input_image_w // cfg.down_ratio, cfg.num_classes),
        dtype=np.float32)
    ...
    batch_ori_wh = np.zeros((cfg.batch_size, 2), dtype=np.int32)
    batch_id = np.zeros((cfg.batch_size), dtype=np.int32)
    ...
    for num, line in enumerate(batch_lines):
        image, hm, wh, reg, reg_mask, ind, ori_w, ori_h, line_id = process_data(line, use_aug)
        ...
        batch_ind[num, :] = ind
        batch_ori_wh[num, :] = ori_w, ori_h
        batch_id[num] = line_id
    return batch_image, batch_hm, batch_wh, batch_reg, batch_reg_mask, batch_ind, batch_ori_wh, batch_id
