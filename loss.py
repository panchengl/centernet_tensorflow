...
def focal_loss_pcl(hm_pred, hm_true):
	pos_mask = tf.cast(tf.equal(hm_true, 1.), dtype=tf.float32)
	neg_mask = tf.cast(tf.less(hm_true, 1.), dtype=tf.float32)
	neg_weights = tf.pow(1. - hm_true, 4)

	pos_loss = tf.log(hm_pred) * tf.pow(1. - hm_pred, 2) * pos_mask
	neg_loss = tf.log(1. - hm_pred) * tf.pow(hm_pred, 2.0) * neg_weights * neg_mask

	num_pos = tf.reduce_sum(pos_mask)
	pos_loss = tf.reduce_sum(pos_loss)
	neg_loss = tf.reduce_sum(neg_loss)

	loss = tf.cond(tf.greater(num_pos, 0), lambda : 0 - (pos_loss + neg_loss) / num_pos, lambda : 0 - neg_loss)
	return loss
...
def reg_l1_loss_pcl(y_pred, y_true, indices, mask):
	# y_pred = _transpose_and_gather(y_pred, indices)
	# mask = tf.tile(tf.expand_dims(mask, axis=-1), (1, 1, 2))
	# b = tf.shape(y_pred)[0]
	# k = tf.shape(indices)[1]
	# c = tf.shape(y_pred)[-1]
